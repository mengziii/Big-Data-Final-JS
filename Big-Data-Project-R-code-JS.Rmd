...
eachfold_RF_uni_5 <- pred_NB_uni_5 %>%
  group_by(Resample) %>%
  summarise_at(vars(equal),
               list(Accuracy = mean))
...
model_NB_bow_5 <- train(
  m_bow[,!names(m_bow) %in% c("class")], m_bow$class, "nb", trControl = trainControl(method = "cv", number = 5, savePredictions = TRUE)
)
...
set.seed(8920)
model_NB_uni_5 <- train(
  m_bow[,!names(m_uni) %in% c("class")], m_uni$class, "nb", trControl = trainControl(method = "cv", number = 5, savePredictions = TRUE)
)
...
set.seed(8920)
model_NB_uni <- train(
  m_bow[,!names(m_uni) %in% c("class")], m_uni$class, "nb", trControl = trainControl(method = "cv", number = 10, savePredictions = TRUE)
)
