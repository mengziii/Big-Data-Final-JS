...
pred_RF_tfidf_100 <- model_RF_tfidf_100$pred
pred_RF_tfidf_100$equal <- ifelse(pred_RF_tfidf_100$pred == pred_RF_tfidf_100$obs, 1, 0)
 
eachfold_RF_tfidf_100 <- pred_RF_tfidf_100 %>%
  group_by(Resample) %>%
  summarise_at(vars(equal),
               list(Accuracy = mean))
 
eachfold_RF_tfidf_100
 
#visualize accuracy scores
ggplot(data=eachfold_RF_tfidf_100, aes(x=Resample, y=Accuracy, group=1)) +
  geom_boxplot(color="maroon") +
  geom_point() +
  theme_minimal()
```
...
Third attempt (10-fold, 100 trees)

```{r, warning = FALSE}
set.seed(21)
model_RF_bow_100 <- train(
...
pred_RF_bow_100 <- model_RF_bow_100$pred
pred_RF_bow_100$equal <- ifelse(pred_RF_bow_100$pred == pred_RF_bow_100$obs, 1, 0)
 
eachfold_RF_bow_100 <- pred_RF_bow_100 %>%
  group_by(Resample) %>%
  summarise_at(vars(equal),
               list(Accuracy = mean))
...
#visualize accuracy scores
ggplot(data=eachfold_RF_bow_100, aes(x=Resample, y=Accuracy, group=1)) +
  geom_boxplot(color="maroon") +
  geom_point() +
  theme_minimal()
```
...
pred_RF_uni_100 <- model_RF_uni_100$pred
pred_RF_uni_100$equal <- ifelse(pred_RF_uni_100$pred == pred_RF_uni_100$obs, 1, 0)
 
eachfold_RF_uni_100 <- pred_RF_uni_100 %>%
  group_by(Resample) %>%
  summarise_at(vars(equal),
               list(Accuracy = mean))
...
#visualize accuracy scores
ggplot(data=eachfold_RF_uni_100, aes(x=Resample, y=Accuracy, group=1)) +
  geom_boxplot(color="maroon") +
  geom_point() +
  theme_minimal()
```
...
Bigrams, like unigrams, break sentences in individual; however, bigrams split words into pairs unlike unigrams.
